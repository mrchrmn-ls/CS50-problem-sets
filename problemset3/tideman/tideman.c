...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        ...
        loop = false; // global variable reset for each locking attempt
...
bool check_for_loop(int loser, int winner)
{

    // when called from lock_pairs() loop is false

    for (int i = 0; i < candidate_count; i++)
    {
        if (loop == false && locked[loser][i] == true && i != winner)
        {
            check_for_loop(i, winner);
        }
        else if (loop == false && locked[loser][i] == true && i == winner)
        {
            loop = true;
        }
    }
